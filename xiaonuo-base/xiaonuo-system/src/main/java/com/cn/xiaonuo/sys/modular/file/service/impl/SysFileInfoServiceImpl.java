...
public class SysFileInfoServiceImpl extends ServiceImpl<SysFileInfoMapper, SysFileInfo> implements SysFileInfoService {
	...
	public Long uploadFile(MultipartFile file) {
		...
		try {
			bytes = file.getBytes();
			fileAbsolutePath = fileOperator.storageFile(DEFAULT_BUCKET, finalName, bytes);
		} catch (IOException e) {
		...
		sysFileInfo.setFilePath(fileAbsolutePath); // 2021-03-19：存储绝对路径！
	 ...
	 */
	@Override
	public void previewAbsoluteImg(SysFileInfoParam sysFileInfoParam, HttpServletResponse response) {
		byte[] fileBytes;
		// 获取文件字节码！
		log.info(">>> [SysFileInfoServiceImpl.previewAbsoluteImg] 待预览图片路径：“" + sysFileInfoParam.getFilePath() + "”..");
		fileBytes = FileUtil.readBytes(sysFileInfoParam.getFilePath());
		// 图片类型，则直接输出！
		try {
			// 设置contentType！
			response.setContentType(MediaType.IMAGE_JPEG_VALUE);
     ...
     */
	@Override
	public void previewAbsolutePdf(SysFileInfoParam sysFileInfoParam, HttpServletResponse response) {
		byte[] fileBytes;
		// 获取文件字节码！
		log.info(">>> [SysFileInfoServiceImpl.previewAbsoluteImg] 待预览PDF路径：“" + sysFileInfoParam.getFilePath() + "”..");
		fileBytes = FileUtil.readBytes(sysFileInfoParam.getFilePath());
		// PDF类型，则直接输出！
		try {
			// 设置contentType！
