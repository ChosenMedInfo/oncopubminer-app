...
public class LogManager {
    ...
    public void executeLoginLog(final String account, final String success, final String failMessage) {
        ...
        MailUtils.sendLogMail("[OncoPubMiner - 用户登录] " + account + "完成系统登录操作", JSONUtil.toJsonPrettyStr(sysVisLog));
    }
    ...
    public void executeExitLog(final String account) {
        ...
        // 2021-12-09（V0013-01.00.10-07）：发送提示邮件！
        MailUtils.sendLogMail("[OncoPubMiner - 用户登出] " + account + "完成系统登出操作", JSONUtil.toJsonPrettyStr(sysVisLog));
    }
    ...
    public void executeOperationLog(BusinessLog businessLog, final String account, JoinPoint joinPoint, final String result) {
        ...
        // 2021-12-09（V0013-01.00.10-07）：发送提示邮件！
        if(!businessLog.title().contains("查") && !businessLog.title().contains("树")) { // 不记录查询/查看/树相关日志！
        	MailUtils.sendLogMail("[OncoPubMiner - 用户操作] " + account + "在系统中执行“" + businessLog.title() + "”操作", JSONUtil.toJsonPrettyStr(sysOpLog));
        }
    }
    ...
    public void executeExceptionLog(BusinessLog businessLog, final String account, JoinPoint joinPoint, Exception exception) {
        ...
        // 2021-12-09（V0013-01.00.10-07）：发送提示邮件！
        MailUtils.sendLogMail("[OncoPubMiner - 异常日志] " + account + "在系统中执行“" + businessLog.title() + "”操作时产生异常", JSONUtil.toJsonPrettyStr(sysOpLog));
    }
     ...
     */
    public SysVisLog genBaseSysVisLog() {
        HttpServletRequest request = HttpServletUtil.getRequest();
        if (ObjectUtil.isNotNull(request)) {
            String ip = IpAddressUtil.getIp(request);
            String address = IpAddressUtil.getAddressFkcoder(request);
