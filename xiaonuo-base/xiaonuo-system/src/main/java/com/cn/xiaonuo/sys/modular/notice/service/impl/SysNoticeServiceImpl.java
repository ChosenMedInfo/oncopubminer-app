...
public class SysNoticeServiceImpl extends ServiceImpl<SysNoticeMapper, SysNotice> implements SysNoticeService {
	...
	public PageResult<SysNotice> page(SysNoticeParam sysNoticeParam) {
		...
		if (ObjectUtil.isNotNull(sysNoticeParam)) {
			...
			//根据状态查询
			if (ObjectUtil.isNotEmpty(sysNoticeParam.getStatus())) {
				queryWrapper.lambda().eq(SysNotice::getStatus, sysNoticeParam.getStatus());
			}
			//排序
		...
		//查询未删除的
		queryWrapper.lambda().ne(SysNotice::getStatus, NoticeStatusEnum.DELETED.getCode());
	...
	public PageResult<SysNoticeReceiveResult> received(SysNoticeParam sysNoticeParam) {
		...
		if (ObjectUtil.isNotNull(sysNoticeParam)) {
			...
			//根据状态查询
			if (ObjectUtil.isNotEmpty(sysNoticeParam.getStatus())) {
				queryWrapper.eq("sys_notice.status", sysNoticeParam.getStatus());
			}else {
				queryWrapper.eq("sys_notice.status", 1); // 只查看“发布”状态的公告！
			}
			//排序
			if(ObjectUtil.isAllNotEmpty(sysNoticeParam.getSortBy(), sysNoticeParam.getOrderBy())) {
				queryWrapper.orderBy(true, sysNoticeParam.getOrderBy().equals(CommonConstant.ASC), StrUtil.toUnderlineCase(sysNoticeParam.getSortBy()));
		...
		//查询未删除的、非草稿的
		queryWrapper.in("sys_notice.status", NoticeStatusEnum.PUBLIC.getCode(), NoticeStatusEnum.CANCEL.getCode());
		
		// 排序！
		queryWrapper.orderByDesc("sys_notice.create_time");
