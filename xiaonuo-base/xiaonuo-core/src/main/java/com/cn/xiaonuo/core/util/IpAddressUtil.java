...
public class IpAddressUtil {
     ...
     *
     * @author Quan Xu
     * @date 2022/3/16 09:17
     */
    public static String getAddressFkcoder(HttpServletRequest request) {
    	String resultJson = SymbolConstant.DASH;
    	try {
	        String ip = getIp(request);
	        //如果是本地ip或局域网ip，则直接不查询
	        if (ObjectUtil.isEmpty(ip) || NetUtil.isInnerIP(ip)) {
	        	resultJson = "本地ip/局域网ip";
	        	return resultJson;
	        }
			//获取fkcoder定位api接口
            String strUrl = ConstantContextHolder.getIpGeoApiFkcoder() + ip;
            log.info("[IpAddressUtil] strUrl: “" + strUrl + "”..");
            HttpRequest http = HttpUtil.createGet(strUrl);
            resultJson = http.timeout(3000).execute().body().toString().replace("\r", "").replace("\n", "").replace("\"", "'");
            log.info("[IpAddressUtil] resultJson: “" + resultJson + "”..");
        } catch (Exception e) {
            resultJson = SymbolConstant.DASH;
        }
        return resultJson;
