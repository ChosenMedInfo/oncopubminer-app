...
public class Config {
    ...
    public static String BASE_JAVA_PATH = "src" + FILE_SEP + "main" + FILE_SEP + "java" + FILE_SEP; // quanxu, 2021-01-06: 去掉最前面的分隔符，处理导出的zip文件中的第一个文件夹src前面带有反斜杠（Windows下）的问题！

    /**
     * html文件夹
     */
    //public static String BASE_HTML_PATH = FILE_SEP + "src" + FILE_SEP + "main" + FILE_SEP + "webapp" + FILE_SEP + "templates" + FILE_SEP;
    public static String BASE_HTML_PATH = "src" + FILE_SEP + "main" + FILE_SEP + "webapp" + FILE_SEP + "templates" + FILE_SEP; // quanxu, 2021-01-06: 去掉最前面的分隔符，处理导出的zip文件中的第一个文件夹src前面带有反斜杠（Windows下）的问题！
...
//        htmlPath = BASE_HTML_PATH + FILE_SEP + busName + FILE_SEP;
//        htmlFormPath = BASE_HTML_PATH + FILE_SEP + busName + FILE_SEP;
        
        // quanxu, 2021-01-06: 去掉多余的分隔符！
        mappingPath = mapperPath + "mapping" + FILE_SEP;
        paramPath = BASE_JAVA_PATH + packageNameString  + MODULAR_NAME + FILE_SEP + busName + FILE_SEP + "param" + FILE_SEP;
        servicePath = BASE_JAVA_PATH + packageNameString  + MODULAR_NAME + FILE_SEP + busName + FILE_SEP + "service" + FILE_SEP;
        serviceImplPath = servicePath + "impl" + FILE_SEP;
        htmlPath = BASE_HTML_PATH + busName + FILE_SEP;
        htmlFormPath = BASE_HTML_PATH + busName + FILE_SEP;
